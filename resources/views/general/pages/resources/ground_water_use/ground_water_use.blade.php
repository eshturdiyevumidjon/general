                ...
                <div class="table-responsive">
                    <table id="" class="table table-striped small reestr-tables">
                        ...
                        <tr class="">
                            ...
                            <th scope="col" colspan="2">{{ trans('messages.Water taken from underground sources') }}</th>
                        </tr>
                        <tr>
                            <th scope="col">{{ trans('messages.total') }}</th>
                            <th scope="col">{{ trans('messages.including due to damage to river flow') }}</th>
                        </tr>
                        </thead>
                        <tbody>
                        @foreach ($ground_water_uses as $key=>$ground_water)
                            <tr class="create-daily-form-table text-center">
                                ...
                                @if(\Auth::user()->role->name == 'Administrator' || \Auth::user()->role->name == 'Editor')
                                    ...
                                    @if($ground_water->is_approve)
                                        <td>
                                            <input class="form-control" step="0.01" type="number"  @change="Changes('river_flow',$event.target.value,{{$ground_water->id}})"   value="{{$ground_water->river_flow}}">
                                        </td>
                                    @else
                                        <td>
                                            <input class="form-control alert-danger" step="0.01" type="number"  @change="Changes('river_flow',$event.target.value,{{$ground_water->id}})"   value="{{$ground_water->river_flow}}">
                                        </td>
                                    @endif
                                @else
                                    @if($ground_water->is_approve)
                                    ...
                                    @if($ground_water->is_approve)
                                        <td>
                                            <input disabled class="form-control" step="0.01" type="number"  @change="Changes('river_flow',$event.target.value,{{$ground_water->id}})"   value="{{$ground_water->river_flow}}">
                                        </td>
                                    @else
                                        <td>
                                            <input disabled class="form-control alert-danger" step="0.01" type="number"  @change="Changes('river_flow',$event.target.value,{{$ground_water->id}})"   value="{{$ground_water->river_flow}}">
                                        </td>
                                    @endif
                                @endif
                            </tr>
