            ...
            <div class="row">
                ...
                <div class="col-12">
                    <h4 class="ml-12 mb-12 font-weight-bold text-primary text-uppercase">
                        Сведения о заборах и сбросах воды, км3/год
                    </h4>
                </div>
            </div>
                    ...
                    <div class="form-row align-items-center  ">
                        <div class="form-group col-5 " >
                            ...
                            <select class="form-control" v-model="options" name="sort">
                                ...
                                @endif
                                @if(\Illuminate\Support\Facades\Auth::user()->org_name == 'gidromet' || \Illuminate\Support\Facades\Auth::user()->org_name == 'other'  )
                                    <option selected value="3">таблица 3.Сведения о заборах и сбросах воды, км3/год</option>
                                @endif
                                @if(\Illuminate\Support\Facades\Auth::user()->org_name == 'gidromet' || \Illuminate\Support\Facades\Auth::user()->org_name == 'other'  )
                        ...
                        <div class="col-5">
                            ...
                            <div class="form-row justify-content-end my-3">
                                ...
                                @if(\Illuminate\Support\Facades\Auth::user()->org_name == 'gidromet')
                                    ...
                                    <form action="{{route('general.resource.water_uses.accept')}}" method="post">
                                        @csrf
                    ...
                    <table id="" class="table table-striped small reestr-tables">
                        ...
                        @foreach ($water_uses as $key=>$water_use)
                            <tr class="create-daily-form-table text-center">
                                ...
                                <td><input class="form-control" step="0.01" type="number"  @change="Changes('total_km',$event.target.value,{{$water_use->id}})" value="{{$water_use->total_km}}"></td>
                                @else
                                    <td><input class="form-control alert-danger" step="0.01" type="number"  @change="Changes('total_km',$event.target.value,{{$water_use->id}})" value="{{$water_use->total_km}}"></td>
                                @endif
                                @if($water_use->is_approve)
                                    <td><input class="form-control " step="0.01" type="number"  @change="Changes('river_network',$event.target.value,{{$water_use->id}})" value="{{$water_use->river_network}}"></td>
                                @else
                                    <td><input class="form-control alert-danger" step="0.01" type="number"  @change="Changes('river_network',$event.target.value,{{$water_use->id}})" value="{{$water_use->river_network}}"></td>
                                @endif
                                @if($water_use->is_approve)
                                <td><input class="form-control" step="0.01" type="number"  @change="Changes('inland_rivers',$event.target.value,{{$water_use->id}})"   value="{{$water_use->inland_rivers}}"></td>
                               @else
                                <td><input class="form-control alert-danger" step="0.01" type="number"  @change="Changes('inland_rivers',$event.target.value,{{$water_use->id}})"   value="{{$water_use->inland_rivers}}"></td>
                                @endif
                                @if($water_use->is_approve)
                                <td><input class="form-control" step="0.01" type="number"  @change="Changes('underground_sources',$event.target.value,{{$water_use->id}})" value="{{$water_use->underground_sources}}"></td>
                                @else
                                 <td><input class="form-control alert-danger" step="0.01" type="number"  @change="Changes('underground_sources',$event.target.value,{{$water_use->id}})" value="{{$water_use->underground_sources}}"></td>
                                @endif
                                @if($water_use->is_approve)
                                <td><input class="form-control" step="0.01" type="number"  @change="Changes('from_collector',$event.target.value,{{$water_use->id}})" value="{{$water_use->from_collector}}"></td>
                                @else
                               <td><input class="form-control alert-danger" step="0.01" type="number"  @change="Changes('from_collector',$event.target.value,{{$water_use->id}})" value="{{$water_use->from_collector}}"></td>
                                @endif

                                @else
                                    @if($water_use->is_approve)
                                    <td><input class="form-control" disabled step="0.01" type="number"  @change="Changes('total_km',$event.target.value,{{$water_use->id}})" value="{{$water_use->total_km}}"></td>
                                    @else
                                    <td><input class="form-control alert-danger" disabled step="0.01" type="number"  @change="Changes('total_km',$event.target.value,{{$water_use->id}})" value="{{$water_use->total_km}}"></td>
                                    @endif
                                    @if($water_use->is_approve)
                                    <td><input class="form-control" disabled step="0.01" type="number"  @change="Changes('river_network',$event.target.value,{{$water_use->id}})" value="{{$water_use->river_network}}"></td>
                                    @else
                                    <td><input class="form-control alert-danger" disabled step="0.01" type="number"  @change="Changes('river_network',$event.target.value,{{$water_use->id}})" value="{{$water_use->river_network}}"></td>
                                    @endif
                                   @if($water_use->is_approve)
                                    <td><input class="form-control" disabled step="0.01" type="number"  @change="Changes('inland_rivers',$event.target.value,{{$water_use->id}})"   value="{{$water_use->inland_rivers}}"></td>
                                    @else
                                   <td><input class="form-control alert-danger" disabled step="0.01" type="number"  @change="Changes('inland_rivers',$event.target.value,{{$water_use->id}})"   value="{{$water_use->inland_rivers}}"></td>
                                   @endif
                                            @if($water_use->is_approve)
                                     <td><input class="form-control " disabled step="0.01" type="number"  @change="Changes('underground_sources',$event.target.value,{{$water_use->id}})" value="{{$water_use->underground_sources}}"></td>
                                    @else
                                 <td><input class="form-control alert-danger" disabled step="0.01" type="number"  @change="Changes('underground_sources',$event.target.value,{{$water_use->id}})" value="{{$water_use->underground_sources}}"></td>
                                  @endif
                                   @if($water_use->is_approve)
                                   <td><input class="form-control" disabled step="0.01" type="number"  @change="Changes('from_collector',$event.target.value,{{$water_use->id}})" value="{{$water_use->from_collector}}"></td>
                                   @else
                                   <td><input class="form-control alert-danger" disabled step="0.01" type="number"  @change="Changes('from_collector',$event.target.value,{{$water_use->id}})" value="{{$water_use->from_collector}}"></td>
                                  @endif
                                            @endhasanyrole
